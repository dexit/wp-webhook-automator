...
abstract class RestController extends WP_REST_Controller {
	...
	public function admin_permissions_check() {
		if ( ! current_user_can( 'manage_options' ) ) {
			return new WP_Error(
				'rest_forbidden',
				__( 'You do not have permission to access this resource.', 'hookly-webhook-automator' ),
				array( 'status' => 403 )
			);
		}
		return true;
	...
	protected function validate_required_fields( $request, array $fields ) {
		...
		if ( ! empty( $missing ) ) {
			return $this->error(
				...
				sprintf( __( 'Missing required fields: %s', 'hookly-webhook-automator' ), implode( ', ', $missing ) ),
				400
			);
		}
