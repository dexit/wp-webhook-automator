...
class WebhooksController extends RestController {
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => array(
						'id' => array(
							'description' => __( 'Unique identifier for the webhook.', 'hookly-webhook-automator' ),
							'type'        => 'integer',
							'required'    => true,
						),
					),
	...
	public function get_item( $request ) {
		...
		if ( ! $webhook ) {
			return $this->error( 'webhook_not_found', __( 'Webhook not found.', 'hookly-webhook-automator' ), 404 );
		}
	...
	public function create_item( $request ) {
		...
		if ( ! filter_var( $endpoint_url, FILTER_VALIDATE_URL ) ) {
			return $this->error( 'invalid_endpoint_url', __( 'Invalid endpoint URL.', 'hookly-webhook-automator' ), 400 );
		}
		...
		if ( ! $id ) {
			return $this->error( 'webhook_save_failed', __( 'Failed to save webhook.', 'hookly-webhook-automator' ), 500 );
		}
		...
		return $this->success(
			...
			__( 'Webhook created successfully.', 'hookly-webhook-automator' ),
			201
		);
	}
	...
	public function update_item( $request ) {
		...
		return $this->success(
			$this->prepare_webhook_for_response( $webhook ),
			__( 'Webhook updated successfully.', 'hookly-webhook-automator' )
		);
	}
	...
	public function delete_item( $request ) {
		...
		if ( ! $deleted ) {
			return $this->error( 'webhook_delete_failed', __( 'Failed to delete webhook.', 'hookly-webhook-automator' ), 500 );
		}

		return $this->success( null, __( 'Webhook deleted successfully.', 'hookly-webhook-automator' ) );
	}
	...
	public function toggle_item( $request ) {
		...
		$webhook = $this->repository->find( $id );
		$status  = $webhook->isActive() ? __( 'activated', 'hookly-webhook-automator' ) : __( 'deactivated', 'hookly-webhook-automator' );

		return $this->success(
			$this->prepare_webhook_for_response( $webhook ),
			/* translators: %s: status (activated/deactivated) */
			sprintf( __( 'Webhook %s successfully.', 'hookly-webhook-automator' ), $status )
		);
	}
	...
	public function test_item( $request ) {
		...
		$test_data = array(
			'test'      => true,
			'timestamp' => time(),
			'message'   => __( 'This is a test webhook from Webhook Automator.', 'hookly-webhook-automator' ),
			'post'      => array(
				'id'       => 1,
				'title'    => __( 'Test Post Title', 'hookly-webhook-automator' ),
				'content'  => __( 'Test post content.', 'hookly-webhook-automator' ),
				'excerpt'  => __( 'Test post excerpt.', 'hookly-webhook-automator' ),
				'status'   => 'publish',
				'type'     => 'post',
				'slug'     => 'test-post',
		...
		if ( $result['success'] ) {
			return $this->success(
				...
				__( 'Test webhook sent successfully.', 'hookly-webhook-automator' )
			);
		}
		...
		return $this->error(
			...
			sprintf( __( 'Test webhook failed: %s', 'hookly-webhook-automator' ), $result['error'] ),
			400
		);
	}
	...
	public function duplicate_item( $request ) {
		...
		if ( ! $new_id ) {
			return $this->error( 'webhook_duplicate_failed', __( 'Failed to duplicate webhook.', 'hookly-webhook-automator' ), 500 );
		}
		...
		return $this->success(
			...
			__( 'Webhook duplicated successfully.', 'hookly-webhook-automator' ),
			201
		);
	}
	...
	public function get_collection_params(): array {
		return array(
			'page'         => array(
				'description'       => __( 'Current page of the collection.', 'hookly-webhook-automator' ),
				'type'              => 'integer',
				'default'           => 1,
				'minimum'           => 1,
			...
			'per_page'     => array(
				'description'       => __( 'Maximum number of items to return.', 'hookly-webhook-automator' ),
				'type'              => 'integer',
				'default'           => 20,
				'minimum'           => 1,
				'maximum'           => 100,
			...
			'trigger_type' => array(
				'description'       => __( 'Filter by trigger type.', 'hookly-webhook-automator' ),
				'type'              => 'string',
				'sanitize_callback' => 'sanitize_text_field',
			),
			'is_active'    => array(
				'description' => __( 'Filter by active status.', 'hookly-webhook-automator' ),
				'type'        => 'integer',
				'enum'        => array( 0, 1 ),
			),
		);
	...
	private function get_webhook_params( bool $required = true ): array {
		return array(
			'name'             => array(
				'description'       => __( 'Webhook name.', 'hookly-webhook-automator' ),
				'type'              => 'string',
				'required'          => $required,
				'sanitize_callback' => 'sanitize_text_field',
			),
			'description'      => array(
				'description'       => __( 'Webhook description.', 'hookly-webhook-automator' ),
				'type'              => 'string',
				'sanitize_callback' => 'sanitize_textarea_field',
			),
			'trigger_type'     => array(
				'description'       => __( 'Trigger type identifier.', 'hookly-webhook-automator' ),
				'type'              => 'string',
				'required'          => $required,
				'sanitize_callback' => 'sanitize_text_field',
			),
			'trigger_config'   => array(
				'description' => __( 'Trigger configuration.', 'hookly-webhook-automator' ),
				'type'        => 'object',
			),
			'endpoint_url'     => array(
				'description'       => __( 'Webhook endpoint URL.', 'hookly-webhook-automator' ),
				'type'              => 'string',
				'required'          => $required,
				'format'            => 'uri',
			...
			'http_method'      => array(
				'description' => __( 'HTTP method.', 'hookly-webhook-automator' ),
				'type'        => 'string',
				'default'     => 'POST',
				'enum'        => array( 'GET', 'POST', 'PUT', 'PATCH', 'DELETE' ),
			),
			'headers'          => array(
				'description' => __( 'Custom HTTP headers.', 'hookly-webhook-automator' ),
				'type'        => 'object',
			),
			'payload_format'   => array(
				'description' => __( 'Payload format.', 'hookly-webhook-automator' ),
				'type'        => 'string',
				'default'     => 'json',
				'enum'        => array( 'json', 'form' ),
			),
			'payload_template' => array(
				'description' => __( 'Payload template.', 'hookly-webhook-automator' ),
				'type'        => 'object',
			),
			'secret_key'       => array(
				'description'       => __( 'Secret key for signature.', 'hookly-webhook-automator' ),
				'type'              => 'string',
				'sanitize_callback' => 'sanitize_text_field',
			),
			'is_active'        => array(
				'description' => __( 'Whether webhook is active.', 'hookly-webhook-automator' ),
				'type'        => 'boolean',
				'default'     => true,
			),
			'retry_count'      => array(
				'description'       => __( 'Number of retry attempts.', 'hookly-webhook-automator' ),
				'type'              => 'integer',
				'default'           => 3,
				'minimum'           => 0,
				'maximum'           => 10,
			...
			'retry_delay'      => array(
				'description'       => __( 'Delay between retries in seconds.', 'hookly-webhook-automator' ),
				'type'              => 'integer',
				'default'           => 60,
				'minimum'           => 10,
				'maximum'           => 3600,
