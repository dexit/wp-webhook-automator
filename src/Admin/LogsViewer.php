...
class LogsViewer {
                ...
                <form method="get" action="">
                    ...
                    <div style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                        ...
                        <div>
                            ...
                            <input type="date" name="date_from" id="filter_date_from" value="<?php echo esc_attr($_GET['date_from'] ?? ''); ?>" placeholder="<?php esc_attr_e('From', 'wp-webhook-automator'); ?>">
                        </div>
                        <div>
                            <label for="filter_date_to" class="screen-reader-text"><?php esc_html_e('To date', 'wp-webhook-automator'); ?></label>
                            <input type="date" name="date_to" id="filter_date_to" value="<?php echo esc_attr($_GET['date_to'] ?? ''); ?>" placeholder="<?php esc_attr_e('To', 'wp-webhook-automator'); ?>">
                        </div>
    ...
    private function getFilterCriteria(): array {
        ...
        if (!empty($_GET['status'])) {
            $criteria['status'] = sanitize_text_field($_GET['status']);
        }

        if (!empty($_GET['date_from'])) {
            $criteria['date_from'] = sanitize_text_field($_GET['date_from']) . ' 00:00:00';
        }

        if (!empty($_GET['date_to'])) {
            $criteria['date_to'] = sanitize_text_field($_GET['date_to']) . ' 23:59:59';
        }
