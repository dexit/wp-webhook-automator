					...
					<div style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
						<div>
							...
							<select name="webhook_id" id="filter_webhook">
								<option value=""><?php esc_html_e( 'All Webhooks', 'hookly-webhook-automator' ); ?></option>
						...
						<div>
							...
							<select name="status" id="filter_status">
								...
								<option value="success" <?php selected( isset( $_GET['status'] ) && $_GET['status'] === 'success' ); ?>><?php esc_html_e( 'Success', 'hookly-webhook-automator' ); ?></option>
								<option value="failed" <?php selected( isset( $_GET['status'] ) && $_GET['status'] === 'failed' ); ?>><?php esc_html_e( 'Failed', 'hookly-webhook-automator' ); ?></option>
								<option value="pending" <?php selected( isset( $_GET['status'] ) && $_GET['status'] === 'pending' ); ?>><?php esc_html_e( 'Pending', 'hookly-webhook-automator' ); ?></option>
							</select>
						</div>
						<div>
							<label for="filter_date_from" class="screen-reader-text"><?php esc_html_e( 'From date', 'hookly-webhook-automator' ); ?></label>
							<input type="date" name="date_from" id="filter_date_from" value="<?php echo isset( $_GET['date_from'] ) ? esc_attr( sanitize_text_field( wp_unslash( $_GET['date_from'] ) ) ) : ''; ?>" placeholder="<?php esc_attr_e( 'From', 'hookly-webhook-automator' ); ?>">
						</div>
						<div>
							<label for="filter_date_to" class="screen-reader-text"><?php esc_html_e( 'To date', 'hookly-webhook-automator' ); ?></label>
							<input type="date" name="date_to" id="filter_date_to" value="<?php echo isset( $_GET['date_to'] ) ? esc_attr( sanitize_text_field( wp_unslash( $_GET['date_to'] ) ) ) : ''; ?>" placeholder="<?php esc_attr_e( 'To', 'hookly-webhook-automator' ); ?>">
						</div>
					...
					<table class="wwa-table">
						<thead>
							<tr>
								...
								<th><?php esc_html_e( 'Trigger', 'hookly-webhook-automator' ); ?></th>
								<th style="width: 80px;"><?php esc_html_e( 'Status', 'hookly-webhook-automator' ); ?></th>
								<th style="width: 80px;"><?php esc_html_e( 'Response', 'hookly-webhook-automator' ); ?></th>
								<th style="width: 80px;"><?php esc_html_e( 'Duration', 'hookly-webhook-automator' ); ?></th>
								<th style="width: 60px;"><?php esc_html_e( 'Attempt', 'hookly-webhook-automator' ); ?></th>
								<th><?php esc_html_e( 'Time', 'hookly-webhook-automator' ); ?></th>
								<th style="width: 100px;"><?php esc_html_e( 'Actions', 'hookly-webhook-automator' ); ?></th>
							</tr>
						</thead>
						<tbody>
