...
class WebhookList {
                ...
                <form method="get" action="">
                    ...
                    <div style="display: flex; gap: 15px; align-items: center;">
                        ...
                        <div>
                            ...
                            <input type="text" name="search" id="filter_search" placeholder="<?php esc_attr_e('Search...', 'wp-webhook-automator'); ?>" value="<?php echo esc_attr($_GET['search'] ?? ''); ?>">
                        </div>
                                    ...
                                    <td>
                                        <?php if ($stats['last_run']) : ?>
                                            ...
                                            <small class="wwa-text-muted">
                                                <?php
                                                printf(
                                                    esc_html__('%d success, %d failed', 'wp-webhook-automator'),
                                                    $stats['success'],
                                                    $stats['failed']
                                                );
                                                ?>
    ...
    private function getFilterCriteria(): array {
        ...
        if (!empty($_GET['trigger_type'])) {
            $criteria['trigger_type'] = sanitize_text_field($_GET['trigger_type']);
        }

        if (!empty($_GET['search'])) {
            $criteria['search'] = sanitize_text_field($_GET['search']);
        }
